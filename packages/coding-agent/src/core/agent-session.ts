...
export class AgentSession {
	...
	async retryLastTurn(): Promise<void> {
		...
		// Durably omit the failed assistant message from the model context. It
		// remains in the session log, so history/export keep a record of the
		// failure, while the retry request sees the same context a turn that
		// simply succeeded later would have seen.
		this._omitRecoveryAttempt(failed);
